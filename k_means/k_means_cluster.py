...
finance_features.sort(key=lambda x:x[1])
print "min exp ", finance_features[0][1]
print "max exp ", finance_features[-1][1]
finance_features.sort(key=lambda x:x[0])
print "min salary ", finance_features[0][0]
print "max salary ", finance_features[-1][0]
print 
...
kmeans = KMeans(n_clusters=5, random_state=10).fit(finance_features)
